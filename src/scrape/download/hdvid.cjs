...
async function handleHdvideo({ hisoka, m, query, tolak, logCommand, fs, path, quoted, downloadMediaMessage }) {
	try {
		const { hdvideo }           = exports;
		const { sparkpixHdUpscale } = require(path.resolve('./src/scrape/ai/sparkpix.cjs'));
		const { hdr: iloveimgHdr }  = require(path.resolve('./src/scrape/ai/iloveimg.cjs'));
