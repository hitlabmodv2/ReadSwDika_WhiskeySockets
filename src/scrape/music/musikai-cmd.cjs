...
async function handleMusicAICallbacks({
        ...
        if (/^__musikai_rlang__(id|jp|en)$/.test(txt)) {
                ...
                const { _GENRES, _GENRES_JP, _GENRES_EN } = require(path.resolve('./src/scrape/music/chatmusic.cjs'));
        ...
        if (txt.startsWith('__musikai_model__')) {
                ...
                const { MODELS: MusicModels } = require(path.resolve('./src/scrape/music/chatmusic.cjs'));
