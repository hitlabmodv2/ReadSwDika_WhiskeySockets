...
**How to apply:** whenever adding a new lazy-required .cjs feature file, register it in WATCHED_FILES the same turn, so future edits to it take effect without asking the user to restart. During multi-step removals, hot reload may run between edits; remove callers before deleting a shared helper or make the change atomically.
