 ...
 * ───────────────────────────────
 *
 *  viewonce2.cjs — View once handler (kirim ke nomor bot sendiri)
 *  Perintah .rvo2 untuk buka pesan view once & kirim ke DM bot
 * ───────────────────────────────
 */
'use strict';
...
// ─────────────────────────────────────────────────────────────────────────────
//  Ambil JID nomor bot sendiri (aman untuk main bot maupun jadibot)
//  hisoka.user.id format: "6289xxx:99@s.whatsapp.net"
//  → strip ":xx" bagian → "6289xxx@s.whatsapp.net"
// ─────────────────────────────────────────────────────────────────────────────

function getBotSelfJid(hisoka) {
        const rawId = hisoka?.user?.id || '';
        if (!rawId) return null;
        const num = rawId.split('@')[0].split(':')[0];
        return num ? `${num}@s.whatsapp.net` : null;
}

async function handleVo2({ hisoka, m, query, tolak, logCommand, quoted, downloadMediaMessage, isJidGroup, hasViewOnceCache, getViewOnceCache }) {
        try {
                if (!m.isQuoted) {
                        if (query) return;
                        await tolak(hisoka, m, `*📱 Cara Penggunaan View Once 2*

*Command:* .rvo2
*Action:* Reply pesan view once yang ingin dibuka

*Perbedaan dengan .rvo:*
• _.rvo_ → media dikirim balik ke chat asal (GC/PM)
• _.rvo2_ → media dikirim ke *DM nomor bot itu sendiri*
...
2. Ketik: .rvo2
3. Media dikirim langsung ke DM bot — aman & privat`);
                        logCommand(m, hisoka, m.command || 'rvo2');
                        return;
                }

                // Pastikan ada JID bot sendiri
                const botSelfJid = getBotSelfJid(hisoka);
                if (!botSelfJid) {
                        await tolak(hisoka, m, '❌ Gagal mendapatkan nomor bot. Coba lagi.');
                        return;
                }
                ...
                // Kirim ke DM nomor bot sendiri — main bot ke DM-nya, jadibot ke DM-nya sendiri
                ...
                console.error('\x1b[31m[RVO2] Error:\x1b[39m', error.message);
                await hisoka.sendMessage(m.from, { react: { text: '❌', key: m.key } });
                await tolak(hisoka, m, `Gagal membuka view once: ${error.message}`);
        }
}

module.exports = { handleVo2 };
