...
async function handleCarilagu({ hisoka, m, query, tolak, logCommand, logError }) {
        try {
                ...
                const report  = _fmt(results, input, pfx);
...
async function handleDetailgenius({ hisoka, m, query, tolak, logCommand, logError }) {
        try {
                const input = (query || '').trim();
                const pfx   = m.prefix || '.';
...
module.exports.handleCarilagu = handleCarilagu;
module.exports.handleDetailgenius = handleDetailgenius;